...
void count_different_mapping(std::vector<SAM_entry>& SAM_data, 
							 ...
							 int read_length, int gap_length)
{
    float SAM_size = SAM_data.size();

	SAM_statistics_file<<"Read_Name"<<'\t'<<"Read1_Uniqueness"<<'\t'<<"Read2_Uniqueness"<<std::endl;
    ...
    for(int i=0;i!=SAM_data.size(); ++i)
    {
		...
		// Process a new read different from previous one.
		// Write the new read name into statistics file.
		//if(i==0 || SAM_data[i].qname != SAM_data[i-1].qname) //Can not process duplicated read-pairs!!
		if(i % 2 ==0)//Start of a new read pair(Row 1, 3, 5, 7, ...)
		{
			...
			else // map to wrong position in SAM(Must be nonUniq_wrong!!)
			{
				...
				{
					if(SAM_data[i].detailed_annotation.find("XT:A:R") != std::string::npos) //With "XT:A:R"
					{
						SAM_statistics_file<<"nonUniq_wrong_scored"; // Only exist in paired-end SAM(one read uniq, the other not uniq but scored.)
					}
					else
					{
			...
			SAM_statistics_file<<'\t'; // Prepare for read2
		}


        

		// Processed Read2 of a paired-end read
		else
		{
			// get theory chromosome name and start coordinate
			extract_chrom_name_positions_from_qname(SAM_data[i].qname, chrom_name, start, end);
			...
			if(chrom_name == SAM_data[i].ref_seq_name && (start + read_length + gap_length == SAM_data[i].position) )
			{//map to right position in SAM
				if(SAM_data[i].map_quality == 0) //multiple mapping(nonUniq)
				{//map_quality==0, sometimes can still be unique mapping. Check "XT:A:U" for unique mapping
					if(SAM_data[i].detailed_annotation.find("XT:A:U") != std::string::npos) //With "XT:A:U"
			...
			else // map to wrong position in SAM(Theoratically, Must be nonUniq_wrong!!)
			{
				if(SAM_data[i].map_quality == 0) //multiple mapping(nonUniq)
				{//map_quality==0, sometimes can still be unique mapping. Check "XT:A:U" for unique mapping
					if(SAM_data[i].detailed_annotation.find("XT:A:U") != std::string::npos) //With "XT:A:U"
			...
			SAM_statistics_file<<std::endl; // Finished read2, prepare for read of next insert.
		}
...
count_error_mapping_rate SAM_filename read_length gap_length
Output:
SAM_filename.different_mapping_count.tsv: check whether read1 and read2 of each read is uniq or not.
 * **********************************/
int main(int argc, char* argv[])
{
    std::string SAM_filename = argv[1];
	int read_length=std::stoi(argv[2]);
	int gap_length = std::stoi(argv[3]);

	std::cout<<"read_length= "<<read_length<<std::endl;
	std::cout<<"gap_length= "<<gap_length<<std::endl;
	...
	count_different_mapping(whole_SAM_data, output_tsv, read_length, gap_length);
